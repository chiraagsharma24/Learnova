...
import {
    ChevronLeft, Play, FileText, Image,
    HelpCircle, CheckCircle2, Menu, X, ArrowLeft,
    ArrowRight, Maximize, MessageSquare
} from "lucide-react";
import ReactPlayer from "react-player";
import {
    fetchCourse, fetchLesson, completeLesson,
    fetchCourseProgress, fetchReviews, submitReview
} from "../api";
        ...
        <div className="flex h-screen bg-white">
            ...
            <aside className={cn(
                ...
                <div className="p-6 border-b border-slate-200 bg-white">
                    <Link to={`/courses/${courseId}`} className="text-xs font-bold text-indigo-600 uppercase tracking-widest flex items-center gap-2 mb-2 group">
                        ...
                        return (
                            <NavLink
                                ...
                                className={cn(
                                    ...
                                    active ? "bg-white border border-slate-200 shadow-sm text-indigo-600 font-bold" : "text-slate-600 hover:bg-slate-100 border border-transparent"
                                )}
                            >
                                <div className={cn(
                                    "w-8 h-8 rounded-lg flex items-center justify-center shrink-0 transition-colors",
                                    completed ? "bg-emerald-100 text-emerald-600" : (active ? "bg-indigo-100 text-indigo-600" : "bg-slate-200 text-slate-400 group-hover:bg-slate-300")
                                )}>
            ...
            <main className="flex-1 flex flex-col overflow-hidden relative">
                    ...
                    {lesson.type === "document" && (
                        <div className="max-w-4xl mx-auto p-8 md:p-16">
                            <div className="mb-10 text-center">
                                <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest mb-4 inline-block">Study Document</span>
                                <h1 className="text-4xl font-black text-slate-800 leading-tight">{lesson.title}</h1>
                            </div>
                            {lesson.documentUrl?.endsWith('.pdf') ? (
                                <iframe src={lesson.documentUrl} className="w-full h-[800px] rounded-3xl border border-slate-200 shadow-xl" />
                            ) : (
                                <div className="bg-white rounded-3xl border border-slate-100 shadow-xl p-10 prose prose-indigo max-w-none">
                                    <p className="leading-relaxed text-slate-600 text-lg">
                                        Browse the document at: <a href={lesson.documentUrl} target="_blank" className="text-indigo-600 font-bold underline">{lesson.documentUrl}</a>
                                    </p>
                ...
                <div className="h-20 border-t border-slate-200 px-8 flex items-center justify-between bg-white/80 backdrop-blur">
                    ...
                    <div className="hidden md:flex flex-col items-center">
                        ...
                        <div className="font-bold text-indigo-600">{currentIdx + 1} of {lessons.length} Lessons</div>
                    </div>
