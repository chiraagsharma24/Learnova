...
import { Plus, Edit2, Trash2, Eye, FileText, Users, MessageSquare } from "lucide-react";
import { Link } from "react-router-dom";
import { fetchAdminCourses, deleteCourse, updateCourse } from "@/fetchers/course";
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 gap-4">
        {courses?.map((course) => (
          <div
            key={course._id}
            className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col md:flex-row md:items-center"
          >
            ...
            <div className="p-5 flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400 font-bold uppercase tracking-wider">
                <span className="flex items-center gap-1">
                  <Users className="w-3 h-3" /> {course.learnerCount} students
                </span>
                <span className="flex items-center gap-1">
                  <MessageSquare className="w-3 h-3" /> {course.reviewCount} reviews
                </span>
            ...
            <div className="p-5 md:pl-0 flex items-center gap-2">
              ...
              <button
                ...
                className={cn(
                  ...
                  course.active
                    ? "bg-white text-emerald-500 hover:bg-emerald-50 border border-emerald-100"
              ...
              <button
                onClick={() => {
                  if (confirm("Are you sure?")) deleteMutation.mutate(course._id);
                }}
