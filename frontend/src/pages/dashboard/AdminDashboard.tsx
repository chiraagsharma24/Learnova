...
import { Users, BookOpen, GraduationCap, Check, X, Clock, BarChart3, ShieldCheck } from "lucide-react";
    ...
    <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-8 pb-16">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-5 gap-3 md:gap-4">
        {statCards.map((item) => (
          ...
          >
            <div
              className={cn("mb-4 inline-flex h-11 w-11 items-center justify-center rounded-xl ring-1", item.accent)}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-8">
        ...
        <aside className="lg:col-span-2 space-y-6">
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-5 text-white shadow-lg">
            <h3 className="text-[11px] font-black uppercase tracking-widest text-indigo-300 mb-4">
              Recent enrollments
            </h3>
          ...
          <div className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            {stats?.latestCourses && stats.latestCourses.length > 0 ? (
              <ul className="space-y-3">
                {stats.latestCourses.map(
                  (course: { _id: string; title?: string; image?: string; createdAt?: string }) => (
