...
import { fetchAdminCourses } from "@/fetchers/course";
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden mb-10">
        <div className="px-6 py-5 border-b border-slate-100 flex items-center justify-between">
          ...
          <Link to="/instructor/courses" className="text-xs font-black text-indigo-600 uppercase hover:underline">
            View All
          </Link>
        </div>
        <div className="divide-y divide-slate-50">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500">
                    {course.learnerCount} learners • Created {new Date(course.createdAt).toLocaleDateString()}
                  </p>
                </div>
              </div>
              <Link
          ...
          {courses?.length === 0 && <div className="p-12 text-center text-slate-400 text-sm">No courses yet.</div>}
        </div>
      </div>
    </div>
  );
