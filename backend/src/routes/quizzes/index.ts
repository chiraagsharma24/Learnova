    ...
    try {
        ...
        const sanitized = {
            ...
            questions: quiz.questions.map((q) => ({
                _id: q._id,
                question: q.question,
                choices: q.choices.map((c) => ({ _id: (c as any)._id, text: c.text })),
            })),
        };
...
//  quiz with answers
router.get("/admin", requireAuth, requireRole("admin", "instructor"), async (req, res) => {
    try {
        const quiz = await Quiz.findOne({ lessonId: req.params.lessonId });
        if (!quiz) return failure(res, 404, "Quiz not found");
...
router.post("/", requireAuth, requireRole("admin", "instructor"), async (req: AuthRequest, res) => {
    try {
        ...
        if (existing) {
            ...
            existing.attemptRewards = attemptRewards || existing.attemptRewards;
            await existing.save();
...
//  submit quiz answers
router.post("/attempt", requireAuth, async (req: AuthRequest, res) => {
    try {
        ...
        const existingProgress = await LessonProgress.findOne({ userId, lessonId });
        ...
        quiz.questions.forEach((q) => {
            ...
            if (correctChoice && selectedChoice && correctChoice.text === selectedChoice.text) correctCount++;
        });
        ...
        const rewardRule = quiz.attemptRewards.find((r) => r.attempt === attemptNumber) ||
            quiz.attemptRewards[quiz.attemptRewards.length - 1];
        const pointsMultiplier = rewardRule ? rewardRule.pointsPercentage / 100 : 0.1;
        const earnedPoints = Math.round(quiz.totalPoints * (scorePercentage / 100) * pointsMultiplier);
        ...
        // Update total points on user profile
        const allProgress = await LessonProgress.find({ userId, courseId });
        const totalCoursePoints = allProgress.reduce((sum, p) => sum + (p.pointsEarned || 0), 0);

        const allUserProgress = await LessonProgress.find({ userId });
        const totalPoints = allUserProgress.reduce((sum, p) => sum + (p.pointsEarned || 0), 0);
