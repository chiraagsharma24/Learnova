...
router.post("/", requireAuth, async (req: AuthRequest, res) => {
    try {
        ...
        if (!courseId || !rating || !comment) return failure(res, 400, "courseId, rating, and comment required");

        const existing = await Review.findOne({ userId: req.user!.id, courseId });
...
router.get("/course/:courseId", async (req, res) => {
    try {
        const reviews = await Review.find({ courseId: req.params.courseId, approved: true }).sort({ createdAt: -1 });
...
router.get("/course/:courseId/all", requireAuth, requireRole("admin", "instructor"), async (req, res) => {
    try {
...
router.put("/:id/approve", requireAuth, requireRole("admin", "instructor"), async (req, res) => {
    try {
